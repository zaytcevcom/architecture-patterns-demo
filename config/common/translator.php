...
return [
    ...
    Translator::class => static function (ContainerInterface $container): Translator {
        ...
        foreach ($modules as $module) {
            foreach ($locales as $locale) {
                $filename =  __DIR__ . '/../../src/Modules/' . ucfirst($module) . '/Translations/' . strtolower($module) . '.' . $locale . '.php';

                if (!file_exists($filename)) {
                    $filename =  __DIR__ . '/../../src/Modules/' . ucfirst($module) . '/Translations/' . strtolower($module) . '.en.php';
    ...
    'config' => [
        'translator' => [
            'lang' => 'en',
            'cache_dir' => (env('APP_ENV') !== 'dev') ? __DIR__ . '/../../var/cache/translator' : null,
            'resources' => [
                [
                    'xlf',

...
class UnionSphereTranslator
{
    ...
    private function getTranslations(string $locale): array
    {
        ...
        $modelsTranslate = $this->connection->createQueryBuilder()
            ->select(['*'])
            ->from('unions_spheres_translate')
            ->where('lang = :lang')
            ->setParameter('lang', $locale)
            ->executeQuery()
            ->fetchAllAssociative();

        /** @var array{sphere_id: int, value: string} $modelTranslate */
        foreach ($modelsTranslate as $modelTranslate) {
            if (!isset($translates[$modelTranslate['sphere_id']])) {
                $translates[$modelTranslate['sphere_id']] = [
